...
const fetchTranslations = async (text, origin, to = 'en', from = 'nl') => {
...
async function handleRequest(request) {
    const { copy, origin, to } = await request.json();
    const string = copy.map(s => `<string>${s}</string>`).join('')
    let translations = await fetchTranslations(string, origin, to);
